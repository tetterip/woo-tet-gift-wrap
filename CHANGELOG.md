...
### Fixed
- Help tooltips (`desc_tip`) on the settings page were empty. WooCommerce only loads its admin JS (tipTip) on its own screens, so the settings page is now registered via the `woocommerce_screen_ids` filter.
...
### Changed
- The plugin title in admin ("Gift Wrap" in the ttrp.gr Plugins menu, the settings page heading and footer) is no longer translatable and always shows in English, for consistency across TTRP plugins. Customer-facing "Gift Wrap" text (fee line, emails, order notices) is still translated.
- `update-checker.php` passes plugin icons and banners from the update server through to WordPress, so the plugin gets an icon in **Dashboard → Updates** and the details modal.
...
### Fixed
- Remove UTF-8 BOM from `update-checker.php` — the BOM was causing a stray character to be output before headers on sites that use the update checker's directory-rename feature.
...
### Fixed
- Guard `$wp_filesystem` global before calling `move()` in the update checker's `fix_directory_name()` — call `WP_Filesystem()` if not already initialised to prevent a fatal error during plugin updates.
- `release.sh`: resolve staging path to absolute before computing relative ZIP entries; fixes broken ZIP paths on Windows.
...
### Changed
- Polished frontend checkout styles: stronger font weight on checkbox label, focus ring on textarea, divider between checkbox and note area, white background for the field container, smooth border-color transitions.
...
### Changed
- Admin settings footer logo width increased to 40 px and opacity set to 1 in `assets/ttrp-admin.css`.
